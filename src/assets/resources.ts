export const lessons = [
  {
    id: 1,
    title: "How to Learn Guitar",
    description: "How to learn guitar.",
    sections: [
      {
        type: "h3",
        style: "",
        content: "What is your goal?",
      },
      {
        type: "p",
        style: "",
        content:
          "kind of guitar, type of music, ect. explain that I am going to cover the stuff that applies to pretty much everyone and helps you to be well rounded",
      },
      {
        type: "h3",
        style: "",
        content: "What I'm going to cover",
      },
      {
        type: "p",
        style: "",
        content:
          "I am going to cover the stuff that applies to pretty much everyone and helps you to be well rounded",
      },
      {
        type: "p",
        style: "",
        content:
          "Mention the absolute basics, how to hold it learn the parts, ect",
      },
      {
        type: "p",
        style: "",
        content:
          "layout sections: Guitar specific technique, learning songs, music theory, ear training, feel, improvising, make sure to mention playing with others.",
      },
    ],
  },
  {
    id: 2,
    title: "Essential Music Theory",
    description: "Notes In Music Major Scale How to Make Chords Keys",
    sections: [
      { type: "h3", style: "", content: "Notes In Music" },
      { type: "p", style: "", content: "Notes In Music" },
      { type: "h3", style: "", content: "Major Scale" },
      { type: "p", style: "", content: "Major Scale" },
      { type: "h3", style: "", content: "Minor Scale" },
      { type: "p", content: "Minor Scale" },
      { type: "h3", style: "", content: "Invtervals and How to Make Chords" },
      { type: "p", style: "", content: "Invtervals and How to Make Chords" },
      { type: "h3", style: "", content: "Keys" },
      { type: "p", style: "", content: "Keys" },
    ],
  },
  {
    id: 3,
    title: "Thoughts on Ear Training",
    description: "Thoughts on ear training",
    sections: [
      { type: "h3", style: "", content: "Why ear training" },
      { type: "p", style: "", content: "Why ear training" },
      {
        type: "h3",
        style: "",
        content: "super quick note about the traditiional interval method",
      },
      {
        type: "p",
        style: "",
        content: "super quick note about the traditiional interval method",
      },
      {
        type: "h3",
        style: "",
        content:
          "why I built my ear training system,easy acoustic songs and melodies vs full tracks",
      },
      {
        type: "p",
        style: "",
        content:
          "why I built my ear training system,easy acoustic songs and melodies vs full tracks",
      },
      {
        type: "h3",
        style: "",
        content: "using the pentatonic scale to find the key of a song",
      },
      {
        type: "p",
        style: "",
        content: "using the pentatonic scale to find the key of a song",
      },
    ],
  },
  {
    id: 4,
    title: "Applying Theory to the Fretboard",
    description: "Applying Theory to the Fretboard",
    sections: [
      {
        type: "h3",
        style: "",
        content:
          "Prerequisites Learning the Fretboard, learn the E and A String, Fretboard Octive Hack,",
      },
      {
        type: "p",
        style: "",
        content:
          "Prerequisites Learning the Fretboard, learn the E and A String, Fretboard Octive Hack,",
      },
      { type: "h3", style: "", content: "Barre Chords and the Caged System," },
      { type: "p", style: "", content: "Barre Chords and the Caged System," },
      { type: "h3", style: "", content: "The Pentatonic Scale," },
      { type: "p", style: "", content: "The Pentatonic Scale," },
      { type: "h3", style: "", content: "Triads" },
      { type: "p", style: "", content: "Triads" },
      { type: "h3", style: "", content: "Playing the Changes" },
      { type: "p", style: "", content: "Playing the Changes" },
      { type: "h3", style: "", content: "Arpeggios" },
      { type: "p", style: "", content: "Arpeggios" },
      { type: "h3", style: "", content: "OP Exersice my favorite exersize" },
      { type: "p", style: "", content: "OP Exersice my favorite exersize" },
    ],
  },
  {
    id: 5,
    title: "Playing With Others",
    description: "Playing With Others",
    sections: [
      { type: "h3", style: "", content: "Improvising" },
      { type: "p", style: "", content: "improvising" },
      { type: "h3", style: "", content: "Feel and Ears. Musicianship" },
      { type: "p", style: "", content: "feel and ears. Musicianship" },
      { type: "h3", style: "", content: "Nashville Number System" },
      { type: "p", style: "", content: "standard tunes" },
    ],
  },
  {
    id: 6,
    title: "Learning Songs",
    description: "Learning Songs",
    sections: [
      { type: "h3", style: "", content: "The Different Ways to Learn Songs" },
      {
        type: "p",
        style: "",
        content:
          "Videos, By ear, tabs, chord tabs, chord charts, standard notation,",
      },
      { type: "h3", style: "", content: "Youtube" },
      { type: "p", style: "", content: "Youtube" },
      { type: "h3", style: "", content: "Learning Songs By Ear" },
      { type: "p", style: "", content: "Learning songs by ear" },
      { type: "h3", style: "", content: "Tab" },
      { type: "p", style: "", content: "Tabs" },
      { type: "h3", style: "", content: "Chord Charts" },
      { type: "p", style: "", content: "Chord charts" },
      { type: "h3", style: "", content: "Standard Notation" },
      { type: "p", style: "", content: "Standard notation" },
    ],
  },
  {
    id: 7,
    title: "The Importance of the Blues",
    description: "The Importance of the Blues",
    sections: [
      { type: "h3", style: "", content: "The Importance of the Blues" },
      { type: "p", style: "", content: "the importance of the blues" },
    ],
  },
  {
    id: 8,
    title: "My Favorite Online Resources and Teachers",
    description: "My Favorite Online Resources and Teachers",
    sections: [
      {
        type: "h3",
        style: "",
        content: "My Favorite Online Resources and Teachers",
      },
      { type: "p", style: "", content: "the importance of the blues" },
    ],
  },
];

export const songList = {
  "Guitar-easy-with-a-capo": [
    {
      Path: "/Songs/ET_Guitar_Jersey_Giant.mp3",
      Title: "Jersey Giant",
      Tip: "",
      Key: "",
      Chords: "",
    },
    {
      Path: "/Songs/ET_Guitar_Summertime_Blues_Verse.mp3",
      Title: "Summertime Blues Verse",
      Tip: "",
      Key: "",
      Chords: "",
    },
  ],
  "Guitar-intermediate": [
    {
      Path: "/Songs/ET_Guitar_Feelin_Good_Again.mp3",
      Title: "Feelin Good Again",
      Tip: "",
      Key: "",
      Chords: "",
    },
    {
      Path: "/Songs/ET_Guitar_My_Heros_Have_Always_Been_Cowboys.mp3",
      Title: "My Heros Have Always Been Cowboys",
      Tip: "",
      Key: "",
      Chords: "",
    },
  ],
  "Guitar-patrick": [
    {
      Path: "/Songs/Weird/Last_Christmas.m4a",
      Title: "Last Christmas",
      Tip: "",
      Key: "",
      Chords: "",
    },
    {
      Path: "/Songs/Weird/Mistletoe_ftJazzy_G.m4a",
      Title: "Mistletoe ft. Jazzy G",
      Tip: "",
      Key: "",
      Chords: "",
    },
    {
      Path: "/Songs/Weird/Rockin_Around_The_Christmas_Tree.m4a",
      Title: "Rockin Around The Christmas Tree",
      Tip: "",
      Key: "",
      Chords: "",
    },
  ],
};

export const instrumentDifficulties = {
  guitar: [
    "intervals",
    "simple-melody",
    "complex-melody",
    "riffs",
    "key-of-g-diatonic-chords-easy",
    "key-of-g-diatonic-chords-hard",
    "key-of-g-diatonic-chords-with-extensions",
    "any-key-diatonic-chords-easy",
    "any-key-diatonic-chords-hard",
    "any-key-diatonic-chords-with-extensions",
    "no-restrictions",
  ],
  piano: [
    "intervals",
    "simple-melody",
    "complex-melody",
    "riffs",
    "key-of-g-diatonic-chords-easy",
    "key-of-g-diatonic-chords-hard",
    "key-of-g-diatonic-chords-with-extensions",
    "any-key-diatonic-chords-easy",
    "any-key-diatonic-chords-hard",
    "any-key-diatonic-chords-with-extensions",
    "no-restrictions",
  ],
  bass: [
    "intervals",
    "simple-melody",
    "complex-melody",
    "riffs",
    "key-of-g-diatonic-chords-easy",
    "key-of-g-diatonic-chords-hard",
    "key-of-g-diatonic-chords-with-extensions",
    "any-key-diatonic-chords-easy",
    "any-key-diatonic-chords-hard",
    "any-key-diatonic-chords-with-extensions",
    "no-restrictions",
  ],
  "guitar-bass": [
    "key-of-g-diatonic-chords-easy",
    "key-of-g-diatonic-chords-hard",
    "key-of-g-diatonic-chords-with-extensions",
    "any-key-diatonic-chords-easy",
    "any-key-diatonic-chords-hard",
    "any-key-diatonic-chords-with-extensions",
    "no-restrictions",
  ],
  "guitar-bass-piano": [
    "key-of-g-diatonic-chords-easy",
    "key-of-g-diatonic-chords-hard",
    "key-of-g-diatonic-chords-with-extensions",
    "any-key-diatonic-chords-easy",
    "any-key-diatonic-chords-hard",
    "any-key-diatonic-chords-with-extensions",
    "no-restrictions",
  ],
};

export const realSongListRealDifficulty = {
  guitar: {
    intervals: [],
    "simple-melody": [
      {
        Path: "/Songs/ET_MO_A_ElPaso.mp3",
        Title: "El Paso",
        Tip: "test",
        Key: "",
        Chords: "test",
        SongInfo: "test",
      },
      {
        Path: "/Songs/ET_MO_I_GOTTV1.mp3",
        Title: "GOTTV1",
        Tip: "test",
        Key: "",
        Chords: "test",
        SongInfo: "test",
      },
    ],
    "complex-melody": [],
    riffs: [],
    "key-of-g-diatonic-chords-easy": [
      {
        Path: "/Songs/ET_Guitar_1415G.mp3",
        Title: "1415G",
        Tip: "Come on dude really?",
        Key: "G",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_1465G.mp3",
        Title: "1465G",
        Tip: "Come on dude really?",
        Key: "G",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_1524G.mp3",
        Title: "1524G",
        Tip: "Come on dude really?",
        Key: "G",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_1645G.mp3",
        Title: "1645G",
        Tip: "Come on dude really?",
        Key: "G",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_5411G.mp3",
        Title: "5411G",
        Tip: "Come on dude really?",
        Key: "G",
        Chords: "",
      },
    ],
    "key-of-g-diatonic-chords-hard": [],
    "key-of-g-diatonic-chords-with-extensions": [],
    "any-key-diatonic-chords-easy": [
      {
        Path: "/Songs/ET_Guitar_Blue_Chorus.mp3",
        Title: "Blue Chorus",
        Tip: "",
        Key: "",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_Simple_Man.mp3",
        Title: "Simple Man",
        Tip: "",
        Key: "",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_Hurt.mp3",
        Title: "Hurt",
        Tip: "",
        Key: "",
        Chords: "",
      },
    ],
    "any-key-diatonic-chords-hard": [],
    "any-key-diatonic-chords-with-extensions": [],
    "no-restrictions": [
      {
        Path: "/Songs/ET_Guitar_Cavalry.mp3",
        Title: "Cavalry",
        Tip: "",
        Key: "",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_Nobody_Knows_You_When_You're_Down_And_Out.mp3",
        Title: "Nobody Knows You When You're Down And Out",
        Tip: "",
        Key: "",
        Chords: "",
      },
      {
        Path: "/Songs/ET_Guitar_Something.mp3",
        Title: "Something",
        Tip: "",
        Key: "",
        Chords: "",
      },
    ],
  },
  piano: {
    intervals: [],
    "simple-melody": [],
    "complex-melody": [],
    riffs: [],
    "key-of-g-diatonic-chords-easy": [],
    "key-of-g-diatonic-chords-hard": [],
    "key-of-g-diatonic-chords-with-extensions": [],
    "any-key-diatonic-chords-easy": [],
    "any-key-diatonic-chords-hard": [],
    "any-key-diatonic-chords-with-extensions": [],
    "no-restrictions": [],
  },
  bass: {
    intervals: [],
    "simple-melody": [],
    "complex-melody": [],
    riffs: [],
    "key-of-g-diatonic-chords-easy": [
      {
        Path: "/Songs/ET_BO_B_G_15.mp3",
        Title: "G 15",
        Tip: "",
        Key: "G",
        Chords: "",
      },
      {
        Path: "/Songs/ET_BO_B_G_1415.mp3",
        Title: "G 1415",
        Tip: "",
        Key: "G",
        Chords: "",
      },
    ],
    "key-of-g-diatonic-chords-hard": [],
    "key-of-g-diatonic-chords-with-extensions": [],
    "any-key-diatonic-chords-easy": [
      {
        Path: "/Songs/ET_BO_I_A_1415.mp3",
        Title: "A 1415",
        Tip: "",
        Key: "A",
        Chords: "",
      },
    ],
    "any-key-diatonic-chords-hard": [],
    "any-key-diatonic-chords-with-extensions": [
      {
        Path: "/Songs/ET_BO_A_12BB.mp3",
        Title: "12BB",
        Tip: "",
        Key: "",
        Chords: "",
      },
    ],
    "no-restrictions": [],
  },
  "guitar-bass": {
    "key-of-g-diatonic-chords-easy": [],
    "key-of-g-diatonic-chords-hard": [],
    "key-of-g-diatonic-chords-with-extensions": [],
    "any-key-diatonic-chords-easy": [],
    "any-key-diatonic-chords-hard": [],
    "any-key-diatonic-chords-with-extensions": [],
    "no-restrictions": [],
  },
  "guitar-bass-piano": {
    "key-of-g-diatonic-chords-easy": [],
    "key-of-g-diatonic-chords-hard": [],
    "key-of-g-diatonic-chords-with-extensions": [],
    "any-key-diatonic-chords-easy": [],
    "any-key-diatonic-chords-hard": [],
    "any-key-diatonic-chords-with-extensions": [],
    "no-restrictions": [],
  },
};

export const notes = [
  { display: "A", value: "A" },
  { display: "A#/Bb", value: "A#" },
  { display: "B", value: "B" },
  { display: "C", value: "C" },
  { display: "C#/Db", value: "C#" },
  { display: "D", value: "D" },
  { display: "D#/Eb", value: "D#" },
  { display: "E", value: "E" },
  { display: "F", value: "F" },
  { display: "F#/Gb", value: "F#" },
  { display: "G", value: "G" },
  { display: "G#/Ab", value: "G#" },
];

export const scales = {
  note: [0], // Just the root note
  major: [0, 2, 4, 5, 7, 9, 11],
  minor: [0, 2, 3, 5, 7, 8, 10],
  majorPentatonic: [0, 2, 4, 7, 9],
  minorPentatonic: [0, 3, 5, 7, 10],
  majorArpeggio: [0, 4, 7],
  minorArpeggio: [0, 3, 7],
};
export const notesss = [
  "A",
  "A#",
  "B",
  "C",
  "C#",
  "D",
  "D#",
  "E",
  "F",
  "F#",
  "G",
  "G#",
];

export const stringNames = ["E", "B", "G", "D", "A", "E"];
